...
#Region "replay"
    ...
    Private Sub updateReplayFrame()
        ...
        Call m_canvas.UpdatePointCloud(m_highlighter.Points)
        ...
        m_replayText.Text = $"步 {m_replayStep + 1}/{m_replay.Steps} · 活跃 {active:N0} · 累积 {cumulative:N0} / 总 {m_replay.TotalSpikes:N0}"
    ...
    Private Sub updateReplayPanel()
        ...
        m_replayTrack.Enabled = hasReplay
        m_replayFirst.Enabled = hasReplay
        m_replayPrev.Enabled = hasReplay
        m_replayNext.Enabled = hasReplay
        m_replayLast.Enabled = hasReplay
        m_replayPlay.Enabled = hasReplay
        m_replayClear.Enabled = hasReplay
        m_replaySpeed.Enabled = hasReplay

        If Not hasReplay Then
            m_replayText.Text = If(m_stimulateMode.Checked,
                                   "在神经元上按住左键并松开即可施加电刺激",
                                   "勾选工具条上的「电刺激模式」后可用")
        ...
        m_replayTrack.Maximum = System.Math.Max(1, m_replay.Steps - 1)

        If m_replayStep >= 0 Then
            m_replayTrack.Value = System.Math.Min(m_replayStep, m_replayTrack.Maximum)
        End If

        m_replayPlay.Text = If(m_replayPlaying, "暂停", "播放")
    End Sub
    ...
    Private Sub onReplayPlay(sender As Object, e As EventArgs)
        ...
        Else
            ...
            m_replayPlaying = True
            m_replayTimer.Interval = CInt(System.Math.Max(20, m_replaySpeed.Value))
            m_replayTimer.Start()
        End If
    ...
    Private Sub onReplayTrackScroll(sender As Object, e As EventArgs)
        ...
        m_replayStep = m_replayTrack.Value
        Call updateReplayFrame()
    End Sub
    ...
    Private Sub rebindHighlighter()
        If m_scene Is Nothing OrElse m_dataset Is Nothing OrElse m_replay Is Nothing Then Return
...
#Region "offline snapshot"
    ...
    Private Sub applySnapshotStimulus()
        ...
        Dim output As String = m_snapshotPath
