...
Public Class AboutForm
    ...
    Private Shared Function buildAboutText() As String
        ...
        Call sb.AppendLine("  Neuropils.exe --dump <转储包.zip> [数据目录] [verify]")
        Call sb.AppendLine("      把数据源的 csv 逐个转储成 msgpack 并打包；带 verify 时会与 csv 路径逐项比对")
        Call sb.AppendLine("      转储包存在且没过期时界面会直接用它加载（省掉全部文本解析）")
