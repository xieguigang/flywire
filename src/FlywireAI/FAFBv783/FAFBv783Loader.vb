...
Namespace FAFBv783
    ...
    Public Module FAFBv783Loader
        ...
        ''' 
        ''' (``DataStream.OpenHandle`` + ``DataStream.AsLinq``，这两个函数都定义在
        ...
        ''' <remarks>
        ''' 数据行以惰性迭代的方式进行读取，内存占用为 O(1)；在迭代结束或者被提前中断的时候
        ''' 由 ``Using`` 语句关闭文件句柄。
        ''' 
        ''' 注意: 在 2026-09-23 之前版本的 ``DataStream`` 之中存在两个缺陷 (目前已经修复):
        ''' 
        ''' 1. ``BufferProvider`` 在 ``BaseStream.Seek(0)`` 之后没有调用 ``StreamReader.DiscardBufferedData()``，
        ''' 导致文件开头缓冲区之中的数据行会被重复读取一次 (例如 ``names.csv`` 返回 139,282 行而不是 139,255 行)；
        ''' 2. ``Dispose`` 没有关闭内部的 ``StreamReader``，导致文件句柄一直被占用
        ...
        ''' 以惰性迭代的方式打开 csv 文件句柄，并且逐行解析为数据模型对象。
        ''' 
        ''' (``DataLinqStream.OpenHandle`` + ``DataLinqStream.AsLinq``，这两个函数也定义在同一个
        ''' ``Data\DataFrame\Linq\DataStream.vb`` 文件之中，可以作为 <see cref="OpenDataStream(Of T)"/>
        ''' 的替代实现)
        ''' </summary>
        <Extension>
        Public Function OpenDataLinqStream(Of T As {New, Class})(path$,
                                                                Optional encoding As Encoding = Nothing,
                                                                Optional parallel As Boolean = False) As IEnumerable(Of T)

            ' 数据行是 IterateAllLines().Skip(1) 的惰性序列，底层的文件读取器由迭代器的 Using 语句释放
